...
def generate_acceleration_dict(entry_vecs, exit_vecs, t_entry, t_exit, particle_properties,
                               timestep=TIMESTEP, strt_padding=SAMPLE_PADDING, end_padding=SAMPLE_PADDING):
    '''
    def generate_acceleration_dict(entry_vecs, exit_vecs, t_entry, t_exit, particle_properties,
                                   timestep=TIMESTEP, strt_padding=SAMPLE_PADDING, end_padding=SAMPLE_PADDING):
        return output
    ...
    for i, entry_vec in enumerate(entry_vecs.T):
        initial_sample_time = int(t_entry[i]) - int(t_entry[i])%timestep-strt_padding*timestep
        vel_vector = (exit_vecs.T[i]-entry_vec)/(t_exit[i]+t_entry[i])
        final_sample_time = int(t_exit[i]) - int(t_exit[i])%timestep+(end_padding+1)*timestep
        ...
        initial_position = entry_vec-vel_vector*(int(t_entry[i])%timestep+strt_padding*timestep)
        positions = np.matmul(
